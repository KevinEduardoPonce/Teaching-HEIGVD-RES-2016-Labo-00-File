...
public class BufferedIOBenchmark {
	...
	final static long NUMBER_OF_BYTES_TO_WRITE = 1024 * 1024 * 10; // we will write and read 10 MB files
        ...
        final static Character SEPARATOR = ','; //Séparateur(ma version d'excel utilise des ; comme séparateur
        final static String FILE_NAME = "statTime.csv"; //Constant nom du fichier
	...
	private void produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
                                   ...
                                   float time)
        {
            ...
            {   
                ...
                //Ecriture des infos dans le buffer relié au fichier
                os.write(operation + SEPARATOR);
                os.write(ioStrategy.toString() + SEPARATOR);
                os.write(Long.toString(numberOfBytesToWrite) + SEPARATOR);
                os.write(Integer.toString(blockSize)+ SEPARATOR);
                os.write(Float.toString(time) + SEPARATOR);
                os.newLine();        
	...
	private void consumeTestData(IOStrategy ioStrategy, int blockSize) {
		...
		LOG.log(Level.INFO, "  > Done in {0} ms.", temps);
                ...
                //Appel la fonction qui écrit dans le fichier
                recupPourStat("READ", ioStrategy, numberOfBytesRead, blockSize, temps);
	...
	public static void main(String[] args) {
                ...
                FileWriter fw = null;
                BufferedWriter os = null;
                //Création du fichier avec les entêtes
                try
                {                
                    fw = new FileWriter(FILE_NAME);
                    os = new BufferedWriter(fw);
                    os.write("operation" + SEPARATOR);
                    os.write("strategy" + SEPARATOR);
                    os.write("blockSize" + SEPARATOR);
                    os.write("fileSizeInBytes" + SEPARATOR);
                    os.write("durationInMs" + SEPARATOR);
                    os.newLine();     
